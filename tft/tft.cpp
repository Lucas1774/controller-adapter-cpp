...
static bool updateAbstractState(const int button, State &state, BufferState &bufferState, const double resScalingX, const double resScalingY, const Functions &functions) {
    ...
    if (state.mode == MouseMovementWithPadMode::FREE) {
        ...
        state.mouse_target = {x * resScalingX, y * resScalingY};
        return true;
    }
    ...
    static const std::map<MouseMovementWithPadMode, std::function<bool()>> modeToFunction = {
        {MouseMovementWithPadMode::ITEMS, [&]() {
             return functions.computeGridBasedMouseTarget(ITEM_COORDINATES, state.mouse_target, state.itemRow, state.itemColumn, button, resScalingX, resScalingY);
         }},
        {MouseMovementWithPadMode::SHOP, [&]() {
             return functions.computeGridBasedMouseTarget(SHOP_COORDINATES, state.mouse_target, state.shopRow, state.shopColumn, button, resScalingX, resScalingY);
         }},
        {MouseMovementWithPadMode::CARDS, [&]() {
             return functions.computeGridBasedMouseTarget(CARD_COORDINATES, state.mouse_target, state.cardRow, state.cardColumn, button, resScalingX, resScalingY);
         }},
        {MouseMovementWithPadMode::LOCK, [&]() {
             return functions.computeAdjacencyMatrixBasedMouseTarget(LOCK_ADJACENCY_MATRIX, LOCK_COORDINATES, state.mouse_target, state.lockIndex, button, resScalingX, resScalingY);
         }}};
...
void run(std::unordered_map<int, int> &buttonState,
    ...
    const auto INPUT_TO_LOGIC_BEFORE = std::unordered_map<int, std::function<bool()>>{
        {PAD_LEFT, [&]() { return updateAbstractState(PAD_LEFT, state, buffer_state, resScalingX, resScalingY, functions); }},
        {PAD_RIGHT, [&]() { return updateAbstractState(PAD_RIGHT, state, buffer_state, resScalingX, resScalingY, functions); }},
        {PAD_UP, [&]() { return updateAbstractState(PAD_UP, state, buffer_state, resScalingX, resScalingY, functions); }},
        {PAD_DOWN, [&]() { return updateAbstractState(PAD_DOWN, state, buffer_state, resScalingX, resScalingY, functions); }}};
        ...
        while (true) {
            ...
            if (!running) {
                ...
                if (buttonState[L1] == JUST_PRESSED) {
                    ...
                    state.shopColumn = 2;
                    auto [x, y] = SHOP_COORDINATES[0][state.shopColumn];
                    state.mouse_target = {x * resScalingX, y * resScalingY};
                ...
                if (rightJoystick.isXActive || rightJoystick.isYActive) {
                    if (std::chrono::steady_clock::now() - lastUpdateTime > std::chrono::milliseconds(16)) {
                        ...
                        functions.moveMouseRelative(
                            static_cast<int>(round(rightJoystick.x * rightJoystick.sensitivity * 100 * resScalingX)),
                            static_cast<int>(round(rightJoystick.y * rightJoystick.sensitivity * 100 * resScalingY)),
